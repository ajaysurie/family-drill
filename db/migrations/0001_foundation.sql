...
CREATE TYPE "DrillEventType" AS ENUM ('drill.sent', 'drill.revealed', 'lure.engaged', 'drill.failed');
...
CREATE TABLE "Attempt" ("id" text PRIMARY KEY, "memberId" text NOT NULL REFERENCES "Member"("id") ON DELETE CASCADE, "scenarioId" text NOT NULL, "drillToken" text NOT NULL UNIQUE, "createdAt" timestamp NOT NULL DEFAULT now(), "scheduledFor" timestamp);
CREATE TABLE "DrillEvent" ("id" text PRIMARY KEY, "attemptId" text NOT NULL REFERENCES "Attempt"("id") ON DELETE CASCADE, "type" "DrillEventType" NOT NULL, "occurredAt" timestamp NOT NULL DEFAULT now(), "summary" text NOT NULL, UNIQUE ("attemptId", "type"));
CREATE TABLE "BotInstall" ("id" text PRIMARY KEY, "householdId" text NOT NULL UNIQUE REFERENCES "HouseholdAgreement"("id") ON DELETE CASCADE, "token" text NOT NULL UNIQUE, "active" integer NOT NULL DEFAULT 1, "householdPaused" integer NOT NULL DEFAULT 0);
