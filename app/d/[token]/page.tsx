...
export default async function DrillPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ confirmed?: string }> }) {
  ...
  return <section className="card reveal"><span className="eyebrow">This was a family drill</span><h1>No shame—let’s learn from the surprise.</h1><p>Your household agreed to occasional fictional practice emails. This message did not impersonate a real organization and never asked for private information.</p><p className="sample"><b>{scenario.fromName}</b><br/>{scenario.subject}<br/><small>{scenario.preview}</small></p>
    <ul>{scenario.lesson.map((item) => <li key={item}>{item}</li>)}</ul>
    <h2>What a family coach can say</h2><ul><li>“These messages are designed to rush anyone.”</li><li>“Pausing and checking with me is always welcome.”</li><li>“Let’s look at the clues together, without blame.”</li></ul>
