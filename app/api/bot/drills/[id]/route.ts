...
import { findAttemptById, getMembers, scenarios } from "../../../../../lib/store";
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return Response.json({ drillId: drill.id, memberName: member?.name ?? "Former household member", scenarioLabel: scenario.subject, lessonBullets: scenario.lesson, lureDeliberate: true, lureEngagementMethod: "explicit_button" });
}
