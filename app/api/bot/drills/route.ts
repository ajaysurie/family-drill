...
export async function POST(request: Request) {
  const auth = await requireBotToken(request); if ("response" in auth) return auth.response;
  ...
  const result = await createBotDrill(auth.bot.organizerId, memberId as string | undefined, scenarioId as string | undefined, sendAt as string | undefined);
  if ("error" in result) {
