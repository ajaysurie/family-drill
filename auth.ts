...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [Nodemailer({
    ...
    from: process.env.EMAIL_FROM_AUTH ?? process.env.EMAIL_FROM ?? "Family Drill <login@localhost>",
    async sendVerificationRequest({ identifier, url, provider }) {
      if (!provider.from) throw new Error("An auth email sender is required");
      await sendAuthVerificationEmail({ identifier, url, from: provider.from });
    },
  })],
  events: {
